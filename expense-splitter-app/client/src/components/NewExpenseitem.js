...
import { getAllPayeeNames } from "../service/expense-util";
...
const NewExpenseItem = ({ expenseItems }) => {
  ...
  const handleExpenseCreation = async (event) => {
    ...
    const expenseDateValue = new Date(expenseDateRef.current.value);
      ...
      <Form onSubmit={handleExpenseCreation}>
        ...
        <Form.Group className="mb-3" controlId="payeeName">
          ...
          <Form.Select aria-label="Default select example" ref={payeeNameRef}>
            <option key="0">SELECT A PAYEE</option>
            {uniquePayeeNames.map((payeeName, index) => {
              return (
                <option key={index + 1} value={payeeName}>
                  {payeeName}
                </option>
              );
            })}
        ...
        <Button variant="primary mr-2" type="submit" onClick={handleClose}>
          Submit
        </Button>
    ...
    return (
      <>
        ...
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>{newExpenseItemForm()}</Modal.Body>
        </Modal>
      </>
    );
  }

  return <div>{newExpenseModal()}</div>;
};
